...
When the topic touches an architectural pattern (API design, concurrency, resilience, observability, security, data access), reference the named pattern from `.openai/skills/chiron/references/engineering-arsenal.md` by name. Load the file on first encounter per session. This gives the user a vocabulary term they can search for independently.
...
When reviewing user code at L3 (signature with blanks) or L4 (full solution), and when the code was generated (by the user or by this assistant), check it against the AI code tells list in `.openai/skills/chiron/references/ai-code-tells.md`. Read that file once per session (first L3/L4 encounter), not on every turn.
...
6. No AI code tells (check against `.openai/skills/chiron/references/ai-code-tells.md`)
