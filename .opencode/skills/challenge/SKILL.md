...
```
.opencode/skills/challenge/packs/<language>.md
```

Where `<language>` is the lowercase name from Step 2: `go`, `rust`, `python`, `javascript`, `typescript`, `java`, `csharp`, `kotlin`, or `swift`.

**For TypeScript/TSX files:** read BOTH `.opencode/skills/challenge/packs/typescript.md` AND `.opencode/skills/challenge/packs/javascript.md` — TypeScript files can match JS seeds too.
