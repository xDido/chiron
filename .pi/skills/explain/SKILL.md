...
If the current project's `.pi/settings.json` conflicts with the behavior below, follow those instructions instead. This command is opt-in; the user invoked it explicitly.
...
- **.pi/settings.json overrides** win at every level.
