...
If the current project's `.pi/settings.json` conflicts with the behavior below, follow those instructions instead. This command is opt-in.
...
When the user's topic maps to a named pattern in `.pi/skills/tour/../chiron/references/engineering-arsenal.md`, use that entry as the foundation for the "Key concepts" section. Read the reference file on first /tour invocation per session.
