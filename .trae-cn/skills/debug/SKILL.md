---
name: debug
description: Structured debugging with hypothesis testing. Observe, hypothesize, verify, fix, explain — teaches debugging methodology via a domain-adapted hint ladder. Defers to .trae-cn/rules.
---
...
Map symptoms to a root cause category from `.trae-cn/skills/debug/references/debugging-playbook.md`. Read the playbook on first L1 encounter per session. Name the category without naming the specific cause.
...
5. No AI code tells (check against `.trae-cn/skills/debug/../chiron/references/ai-code-tells.md`)
