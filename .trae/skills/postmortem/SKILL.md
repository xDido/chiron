...
If the current project's `.trae/rules` says to skip session reviews or use different scoring, follow those instructions instead. This command is opt-in.
...
Reviews the current Trae conversation's recent chiron activity and presents a session-end report. Based on the *"After each session"* pattern from `go-mentor.md`:
