...
- `{{command_prefix}}architect` — no argument: infer the design decision from the current conversation
...
- If the decision would be better served by a light comparison than a formal ADR, redirect: *"That's a lighter decision — try `{{command_prefix}}explain` instead."* `{{command_prefix}}architect` is for decisions worth committing to a record.
- If no architecture topic is visible (the conversation is about debugging, implementation details, or unrelated), stop with: *"No architecture decision visible in conversation — try `{{command_prefix}}architect <decision>`."*
