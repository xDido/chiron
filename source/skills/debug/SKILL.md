...
- If no failure signal is visible (the conversation is about design, not bugs), stop with: *"No error visible in conversation — try `{{command_prefix}}debug <error description or file:line>`."*
- Never fabricate a bug to fill the slot. Ambiguity → ask, don't guess.
- **Never-refuse rule still applies:** if the user's last message includes *"just fix it"* or equivalent alongside a bare `{{command_prefix}}debug`, infer the bug AND ship the fix immediately — don't make them restate.
